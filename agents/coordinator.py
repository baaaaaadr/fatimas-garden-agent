...
coordinator = LlmAgent(
    ...
    instruction=(
        ...
        "- If a specialist says a product is out of stock and suggests an alternative, KEEP that in your "
        "reply: say you would normally recommend the out-of-stock product but it is currently "
        "unavailable, so you suggest the in-stock alternative instead.\n"
        ...
        "`send_reply` returns: if its status is 'sent', show the customer the final reply (output the "
        "exact reply text that was sent). If its status is 'held', tell the customer the reply was "
        "held for a human to edit.\n"
